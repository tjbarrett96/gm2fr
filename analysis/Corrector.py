...
class Corrector:
  ...
  def correct(self, distortion = True, background = False, wiggle = False):
    ...
    self.wiggle = self.truth_frequency.copy().clear().setHeights(self.scale * calc.s(self.truth_frequency.centers, self.transform.start, self.transform.end, self.transform.t0))
  ...
  def plot(self, output_path):
    ...
    style.labelAndSave("Frequency (kHz)", "Coefficient", f"{output_path}/Coefficients.pdf")
    ...
    plt.xlim(const.info["f"].min, const.info["f"].max)
    style.labelAndSave("Frequency (kHz)", "Scaled Distribution", f"{output_path}/ScaledDistributions.pdf")
    ...
    plt.xlim(const.info["f"].min, const.info["f"].max)
    style.labelAndSave("Frequency (kHz)", "Term", f"{output_path}/Terms.pdf")
    ...
    style.labelAndSave("Frequency (kHz)", "Arbitrary Units", f"{output_path}/Predicted_vs_Actual.pdf")
    ...
    style.labelAndSave("Frequency (kHz)", "Arbitrary Units", f"{output_path}/Truth_vs_Actual.pdf")
    ...
    style.yZero()
    plt.xlim(const.info["f"].min, const.info["f"].max)
    style.labelAndSave("Frequency (kHz)", "Arbitrary Units", f"{output_path}/Truth_vs_Corrected.pdf")
