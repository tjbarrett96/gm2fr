...
# TODO: during the incremental fits, first *fix* the parameters from the previous
# iteration to get better estimates of the new parameters, and then let them all float
def fit(time, signal, errors, model = "nine", n = 0.108):

  # Sequence of increasingly complex fits to perform.
  models = ["two", "five", "nine"]

  # Select the slice up to (and including) the specified model.
  if model in models:
    models = models[:(models.index(model) + 1)]
  else:
    raise ValueError(f"Wiggle fit model {model} not recognized.")

  # Mask by the fit start time (30 us).
  mask = (time >= 30)
  fitTime = time[mask]
  fitSignal = signal[mask]
  fitErrors = errors[mask]

  # Initialize fit parameter and covariance results.
  popt, pcov = None, None
  ...
  for model in models:
    ...
    # Set the CBO frequency seed (it's sensitive) based on the expected n-value.
    if model == "nine":
      seeds[7] = (1 - np.sqrt(1 - n)) * util.magicFrequency * 1E-3
    ...
    # Calculate the best fit and residuals.
    bestFit = function(fitTime, *popt)
    fitResiduals = fitSignal - bestFit
    
    # Calculate the chi-squared, and reduced chi-squared.
    chi2 = np.sum((fitResiduals / fitErrors)**2)
    chi2ndf = chi2 / (len(fitTime) - len(popt))
    
    # Status update.
    print(f"\nCompleted {model}-parameter wiggle fit.")
    print(f"{'chi2/ndf':>16} = {chi2ndf:.4f}")
    for i in range(len(modelLabels[model])):
      if modelLabels[model][i][0] == "frequency":
        continue # don't reveal omega_a
      print(f"{modelLabels[model][i][0]:>16} = {popt[i]:.4f} +/- {pcov[i][i]**0.5:.4f} {modelLabels[model][i][1]}")

  return function, popt, chi2ndf
