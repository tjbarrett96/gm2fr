...
class Transform:
  ...
  def __init__(
    self,
    fastRotation, # gm2fr.FastRotation object containing the signal
    start = 4,    # start time (us)
    end = 200,    # end time (us)
    df = 2,       # frequency spacing (kHz)
    cutoff = 3,   # background definition cutoff (sigma)
    model = "parabola"
  ):
...
#    )
    ...
    # Collimator mask.
    self.mask = (self.frequency >= util.minimum["frequency"]) & (self.frequency <= util.maximum["frequency"])
  ...
  def transform(self, t0):
    ...
    # TODO: do this in the constructor? doesn't need to be re-calculated
    # Mask the signal by the start and end times.
    mask = (self.fr.time >= self.start) & (self.fr.time <= self.end)
    time = self.fr.time[mask]
    signal = self.fr.signal[mask]
    
    # Conversion factor from (kHz * us) to standard units (Hz * s = 1).
    kHz_us = 1E-3
    
    # Pre-compute the cosine argument, except for the frequency factor.
    cosArg = 2 * np.pi * (time - t0) * kHz_us
    
    # Calculate the cosine transform.
    for i in range(len(self.frequency)):
      self.signal[i] = np.sum(signal * np.cos(self.frequency[i] * cosArg))
      
    # Normalize the peak of the transform to one.
  ...
  def optimize(
    ...
    if mode == "fine":
      times = np.arange(seed - 0.0005, seed + 0.0005, 0.000025)
    elif mode == "coarse":
      times = np.arange(seed - 0.015, seed + 0.015, 0.0005)
    else:
    ...
    # TODO: this is redundant if the mode is minimum chi-squared
    optIndex = 0
    for i in range(len(times)):
      if better(scanResults[i], scanResults[optIndex]):
        optIndex = i
        
    # Extract the list of chi-squareds and bounds from the fit objects.
    ...
    # TODO: under "coarse" mode, simply select the t0 at the optIndex; only fit from chi-squared in "fine" mode
    else:
      ...
      if mode == "coarse":
        ...
        # Fit a parabola using the 2 neighbors on either side of the scan minimum.
        popt = np.polyfit(
  ...
  def process(
    ...
    # TODO: if the fixed t0 is bad, bound optimization will fail; find a way to catch and handle this elegantly
      ...
      self.bgFit.chi2dof /= self.bgFit.uncertainty**2
  ...
  def getMean(self, axis = "frequency"):
    return np.average(
      self.axes[axis][self.mask],
      weights = self.signal[self.mask]
    )
  ...
  def getWidth(self, axis = "frequency"):
    ...
    return np.sqrt(
      np.average(
        (self.axes[axis][self.mask] - mean)**2,
        weights = self.signal[self.mask]
      )
    )
  ...
  def getCorrection(self, n = 0.108):
    return util.radialOffsetToCorrection(self.radius[self.mask], self.signal[self.mask], n)
